...
int CountBagsInBag(const std::vector<Bag> Bags, Bag Bag)
{
    int Count = 0;
    ...
    for (const auto Pair : Bag.Contains)
    {
        ...
        {
            Count += CountBagsInBag(Bags, getBagByName(Bags, Pair.second));
        }
    }
    return Count;
}
...
int main()
{
    ...
    std::cout << CountBagsInBag(Bags, getBagByName(Bags, "shiny gold"));
}